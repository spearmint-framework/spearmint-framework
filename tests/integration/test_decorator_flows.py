...
class TestSpearmintInstanceBasedDecoration:
    ...
    def test_spearmint_instance_override_strategy(self, sample_configs):
        ...
        @sp.configure(strategy=MultiBranchStrategy)  # Override with MultiBranch
        async def process(config: Config) -> str:
            return config["id"]
        
        # Function should use MultiBranchStrategy despite instance default
...
class TestDecoratorEdgeCases:
    ...
    async def test_function_with_args_and_kwargs(self, single_config):
        ...
        async def flexible_func(config: Config, *args, **kwargs) -> dict:
            await asyncio.sleep(0.01)
            return {
                "config_id": config["id"],
                "args": args,
